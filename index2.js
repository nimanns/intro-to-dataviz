...
import file from "url:./data.csv";
        ...
        d3.csv(file).then(function(data) {
            ...
            var x = d3.scaleTime()
                .range([0, width])
                .domain(d3.extent(data, function(d) { return new Date(d.Occurred); })); // Use "Occurred" column

            var y = d3.scaleBand()
                .range([height, 0])
                .domain(data.map(function(d) { return d["Country(ies)"]; }))
                .padding(.01);
            ...
            var nestedData = Array.from(d3.group(data, d => x(new Date(d.Occurred)) + "-" + d["Country(ies)"]));
            ...
            svg.selectAll(".circle-group")
                ...
                .attr("cy", function(d) { return y(d["Country(ies)"]) + y.bandwidth() / 2; })
                .attr("r", 15)
                ...
                .on("mouseover", function(d) {
                    ...
                    tooltip.transition()
                        .duration(200)
                        .style("opacity", .9);
                    tooltip.html("Type: " + d.target.__data__.Type + "<br/>" +
                                 "Country: " + d.target.__data__["Country(ies)"] + "<br/>" +
                                 "Occurred: " + d.target.__data__.Occurred)
